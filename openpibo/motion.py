...
서보 모터 정보::
  ...
  *모션 프로파일: 내장된 모션 프로파일 ``set_motion`` 메소드로 프로파일 내의 동작을 수행할 수 있습니다.
  ...
  def move(self, vx, vy, cycle=1):
    """
    방향과 속도를 지정하여 로봇을 이동합니다.

    example::

      motion.move(-0.5, 1.0)   # 왼쪽 앞으로
      motion.move(0, -1.0)     # 뒤로
      motion.move(1.0, 0)      # 우회전
    ...
    for pos in result['pos']:
        d = pos['d']
        if d[1] != 999:
            d[1] = max(-30, min(30, int(d[1] * right_scale)))
        if d[7] != 999:
            d[7] = max(-30, min(30, int(d[7] * left_scale)))
