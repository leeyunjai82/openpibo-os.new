...
tools/             체험 도구 (:50000, /tools/ 로 접근)
  ...
  pibrain/         REST+SSE   — 버튼/LED/카메라/음성/LCD
...
test/              개발용 시험 — 하드웨어 없이 돈다 (bash test/run.sh)
selftest/          기기 검사 — 파이에서, 하드웨어가 있어야 돈다
```
...
경계가 보이지 않는다(시험이 이를 지킨다). `tools/pibo/static/theme.css`,
