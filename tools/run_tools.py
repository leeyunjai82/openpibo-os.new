...
def init_pibo():
  global pibo
  # 이 tools 서버는 **파이보 것**이다 (socket.io, :50000).
  # 파이브레인은 하드웨어가 달라 화면 자체가 다르다 → tools/pibrain/ 에 따로 있다.
  # 어느 쪽을 띄울지는 tools/launch.py 가 보드를 보고 고른다.
  # 그래도 실수로 파이브레인에서 이게 뜨면(직접 실행 등) 로그로 알린다.
  from openpibo.board import BOARD
  if BOARD.name != 'pibo':
    logging.warning(
      f'[tools] 이 서버는 파이보용입니다. 지금 보드는 "{BOARD.label}"({BOARD.name}) 입니다. '
      f'모션/MCU 기능은 동작하지 않습니다. tools/launch.py 로 띄우면 보드에 맞는 앱이 뜹니다.'
    )
