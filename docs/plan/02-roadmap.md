...
1. **vapi-od `view_project` 라우팅 규약 이식**
   - `/<묶음>/<기능>`
   - 응답 봉투 `{type, result, data, elapsed_ms, device}`
2. 상단바 + 탭 (체험 / 블록 / 파이썬 / 학습 / 대화)
3. **설정 모달** — 와이파이, 시스템 정보, 로그, 초기화, 전원
   (현재 `landing.html`에 섞여 있음)
4. 상태 배지 — 카메라 사용중 / 코드 실행중 / 모델 로딩중
5. 반응형 — 교실 태블릿 상정, 상단 탭 → 하단 탭바 브레이크포인트
